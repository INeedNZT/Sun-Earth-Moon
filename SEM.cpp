...
const char *WINDOWTITLE = "OpenGL / Sun-Earth-Moon System";
...
int		WhichView;
int     IsFrozen = 1;
int     IsMouseDisable = 0;
int		IsLightOn = 1;


const float MoonRadius = 1.74;
const float EarthRadius = 3.95;
const float SunRadius = 11.85;


const float EarthOrbitalPeriod = 365 * 24 * 3600;
const float MoonOrbitalPeriod = 27 * 24 * 3600;
const float EarthSpinPeriod = 24 * 3600;
const float MoonSpinPeriod = 27 * 24 * 3600;


const int MoonViewMode = 1;
const int EarthViewMode = 2;
const int OverviewMode = 0;



unsigned char* Texture0;			// the texels0
unsigned char* Texture1;			// the texels1
unsigned char* Texture2;			// the texels2
unsigned char* Texture3;			// the texels3
unsigned char* Texture4;			// the texels4

GLuint  EarthTex;				// the earth day texture object
GLuint  MoonTex;				// the moon texture object
GLuint  MoonDTex;				// the moon displacement texture object
GLuint  SunTex;					// the sun texture object
GLuint	AstronautTex;			// the astronaut texture object

GLuint  EarthList;              // Earth List
GLuint  MoonList;               // Moon List
GLuint  SunList;				// Sun List
GLuint  AstronautList;			// Astronaut List

// collect the current localtion (model view matrix)
GLfloat matrix[16];
...
Animate( )
{
	...
	int ms = glutGet(GLUT_ELAPSED_TIME); // millisecondsms %= MS_IN_THE_ANIMATION_CYCLE;
...
Display( )
{
	...
	float makesmallfactor = 20000000.;
	float EarthOrbitalEqX = 34 * cos(EarthOrbitalPeriod / makesmallfactor * Time * M_PI);
	float EarthOrbitalEqZ = 28 * sin(EarthOrbitalPeriod / makesmallfactor * Time * M_PI);

	float MoonOrbitalEqX = EarthOrbitalEqX + 10 * cos(MoonOrbitalPeriod / makesmallfactor * Time * M_PI);
	float MoonOrbitalEqZ = EarthOrbitalEqZ + 6 * sin(MoonOrbitalPeriod / makesmallfactor * Time * M_PI);

	// self spin 
	float spinsmallfactor = 10000;
	float EarthSpinSpeed = EarthSpinPeriod / spinsmallfactor;
	float MoonSpinSpeed = MoonSpinPeriod / spinsmallfactor;

	float EarthSpinAngle = (int)(3. * Time * EarthSpinSpeed) % 360;
	float MoonSpinAngle = (int)(3. * Time * MoonSpinSpeed) % 360;
	...
	{
	case OverviewMode:
		gluLookAt(0., 40., 55., 0., 0., 0., 0., 10., 0.);
		break;
	// move with the moon
	case MoonViewMode:
		gluLookAt(MoonOrbitalEqX-0.2, MoonRadius + 1.5, MoonOrbitalEqZ + 2, MoonOrbitalEqX, MoonRadius + 0.2, MoonOrbitalEqZ, -0.2, 1, -1);
		break;
	// move with the earth
	case EarthViewMode:
		gluLookAt(EarthOrbitalEqX, EarthRadius + 0.4, EarthOrbitalEqZ + 2, EarthOrbitalEqX + EarthSpinAngle, EarthRadius + 0.2, EarthOrbitalEqZ - 0.2, -0.2, 1, 0);
		break;
	default:
		WhichView = OverviewMode;
	}
	...
	glPushMatrix();
	MPattern->Use();
	MPattern->SetUniformVariable("uDispUnit", uDispUnit);
	MPattern->SetUniformVariable("uColorUnit", uColorUnit);
	MPattern->SetUniformVariable("uScale", uScale);
	MPattern->SetUniformVariable("uLightX", LightX);
	MPattern->SetUniformVariable("uLightY", LightY);
	MPattern->SetUniformVariable("uLightZ", LightZ);
	MPattern->SetUniformVariable("uKd", Kd);
	
	glTranslatef(MoonOrbitalEqX, 0, MoonOrbitalEqZ);
	//glTranslatef(32., 0., 0.);
	glRotatef(MoonSpinAngle, 0., 1., 0);
	...
	// create Earth
	glPushMatrix();
	glTranslatef(EarthOrbitalEqX, 0., EarthOrbitalEqZ);
	//glTranslatef(25., 0., 0.);
	glRotatef(EarthSpinAngle, 0., 1., 0);
	...
	// create astronaut on moon surface
	glPushMatrix();
	glTranslatef(MoonOrbitalEqX, MoonRadius + 0.4, MoonOrbitalEqZ);
	glScalef(0.25, 0.25, 0.25);
	glCallList(AstronautList);
	glPopMatrix();
...
InitGraphics( )
{
	...
	Texture0 = BmpToTexture((char*)"moon.bmp", &width, &height);
	if (Texture0 == NULL)
	...
	Texture1 = BmpToTexture((char*)"moondisp.bmp", &width, &height);
	if (Texture1 == NULL)
		fprintf(stderr, "Cannot open texture '%s'\n", "moondisp.bmp");
	else
		fprintf(stderr, "Width = %d ; Height = %d\n", width, height);
	
	glGenTextures(1, &MoonDTex);
	glBindTexture(GL_TEXTURE_2D, MoonDTex);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	Texture2 = BmpToTexture((char*)"earth_daymap.bmp", &width, &height);
	if (Texture2 == NULL)
		fprintf(stderr, "Cannot open texture '%s'\n", "earth_daymap.bmp");
	else
		fprintf(stderr, "Width = %d ; Height = %d\n", width, height);
	...
	Texture3 = BmpToTexture((char*)"sun.bmp", &width, &height);
	if (Texture3 == NULL)
		fprintf(stderr, "Cannot open texture '%s'\n", "sun.bmp");
	else
		fprintf(stderr, "Width = %d ; Height = %d\n", width, height);
	...
	MPattern = new GLSLProgram();
	valid = MPattern->Create("moondisp.vert", "moondisp.frag");
	if (!valid)
	{
		fprintf(stderr, "Moon displacement Shader cannot be created!\n");
		DoMainMenu(QUIT);
	}
	else
	{
		fprintf(stderr, "Moon displacement created.\n");
	}
	MPattern->SetVerbose(false);
...
InitLists( )
{
	...
	glBindTexture(GL_TEXTURE_2D, MoonTex);
	SetMaterial(1., 0., 0., 15.);
	glShadeModel(GL_FLAT);
	OsuSphere(MoonRadius, 40, 40);
	glDisable(GL_TEXTURE_2D);
	glEndList();


	// Create List for the Earth
	...
	SetMaterial(1., 0., 0., 15.);
	glShadeModel(GL_FLAT);
	OsuSphere(EarthRadius, 40, 40);
	glDisable(GL_TEXTURE_2D);
	glEndList();


	// Create List for the Sun
	SunList = glGenLists(1);
	glNewList(SunList, GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, SunTex);
	glTranslatef(0., 0., 0.);
	OsuSphere(SunRadius, 40, 40);
	glDisable(GL_TEXTURE_2D);
	glEndList();


	// creat List for astronaut
	AstronautList = glGenLists(1);
	glNewList(AstronautList, GL_COMPILE);
	SetMaterial(1., 0., 1., 5.);
	glShadeModel(GL_FLAT);
	glColor3f(1., 1., 1.);
	LoadObjFile("astronaut.obj");
	glEndList();
